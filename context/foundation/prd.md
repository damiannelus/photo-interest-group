...
- No submission, comment, challenge, or user data is accessible to any unauthenticated user or any authenticated user whose email is not on the whitelist — not via the UI, and not via any direct API call.
...
**Auth:** Google Sign-In (OAuth). Any Google account can attempt login; only accounts whose email appears on the membership whitelist may proceed to the app.

**Whitelist management (MVP):** Managed out-of-band. No in-app UI for adding members.
